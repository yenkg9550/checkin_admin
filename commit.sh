#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────
# commit.sh  ─  CheckIn Admin（Vue 3 + Element Plus）
# 用法：
...
if [[ $# -ge 1 && -n "$1" ]]; then
  ...
  SCOPES=()
  echo "$CHANGED_FILES" | grep -q "src/views/Dashboard"     && SCOPES+=("dashboard")
  echo "$CHANGED_FILES" | grep -q "src/views/Login"         && SCOPES+=("login")
  echo "$CHANGED_FILES" | grep -q "src/views/Employees"     && SCOPES+=("employees")
  echo "$CHANGED_FILES" | grep -q "src/views/Settings"      && SCOPES+=("settings")
  echo "$CHANGED_FILES" | grep -q "src/views/Admins"        && SCOPES+=("admins")
  echo "$CHANGED_FILES" | grep -q "src/views/"              && SCOPES+=("views")
  echo "$CHANGED_FILES" | grep -q "src/stores/"             && SCOPES+=("stores")
  echo "$CHANGED_FILES" | grep -q "src/api/"                && SCOPES+=("api")
  echo "$CHANGED_FILES" | grep -q "src/router/"             && SCOPES+=("router")
  echo "$CHANGED_FILES" | grep -q "src/components/"         && SCOPES+=("components")
  echo "$CHANGED_FILES" | grep -q "src/assets/"             && SCOPES+=("assets")
  echo "$CHANGED_FILES" | grep -q "vite\.config\|index\.html\|package\.json\|\.env\|commit\.sh\|\.gitignore" && SCOPES+=("infra")
